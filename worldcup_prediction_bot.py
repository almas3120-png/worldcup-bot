...
bot = Bot(token=TOKEN)
dp = Dispatcher()db = sqlite3.connect("worldcup.db")
...
db.commit()class Registration(StatesGroup):
    first_name = State()
    last_name = State()
    national_code = State()
    mobile = State()
    province = State()
    city = State()
    rules = State()class Registration(StatesGroup):
    first_name = State()
    last_name = State()
    national_code = State()
    mobile = State()
    province = State()
    city = State()
    rules = State()main_menu = ReplyKeyboardMarkup(
    keyboard=[
        [
    ...
    return row and row[0] == 1@dp.message(Command("start"))
async def start(
